...
void WhatsProgDadosServidor::enviarMensagem(iterMensagem iMsg, iterUsuario iDest)
{
  try
  {
    ...
    iterUsuario remetente1= find(LU.begin(), LU.end(), iMsg->getRemetente());
    //remetente2 = find(LU.begin(), LU.end(), iMsg->getRemetente());


    //if(iMsg->getRemetente()== *remetente1 && iMsg->getRemetente()== *remetente2) throw 6;

    if(remetente1==LU.end() ) throw 6;

    if (remetente1->connected()) enviarConfirmacao(iMsg, remetente1);
  }
  catch (int erro)
  {
    if (erro>=2 && erro<=5)
  ...
  while (!fim)
  {
    try // Erros graves: catch encerra o servidor
    {
      ...
      case mysocket_status::SOCK_OK: // resultado do wait_read
        ...
        catch (int erro) // Erros na  do socket de algum cliente
        {
          if (erro>=6 && erro<=9)
          {
