...
Circuito:: Circuito(const Circuito& C):  Nin(C.Nin){
...
{
    // Verifica autopatribuição
    if (this != &C) {
        // Limpa o circuito atual
        clear();

        // Copia Nin, id_out e out_circ
        Nin = C.getNumInputs();
        for (int i = 0; i < C.getNumInputs()-1; i++) {
            id_out.push_back(C.getIdOutput(i));
            out_circ.push_back(C.getOutput(i));
        }

        // Copia as portas usando clone
        for (int i = 0; i < C.getNumPorts(); i++) {
            ports.push_back(C.ports[i]->clone());
        }
    }

    // Retorna a referência para permitir a encadeamento de operadores
    return *this;
}
  ...
  for (i=0; i<getNumOutputs(); ++i)
  {
      int IdOut;
        ...
        while(!validIdOrig(IdOut)){
            std::cerr << "Id digitado eh invalido" << std::endl;
            std::cout << "Digite o Id da saida " << i+1 << ":";
            std::cin >> IdOut;
        }
        id_out.at(i) = (IdOut);
...
// funcao ler da classe Port na porta recem-criada.

//O retorno da leitura da porta eh conferido bem como se a porta lida eh valida (validPort) para o circuito.
...
bool Circuito::ler(const std::string& arq)
{
  ...
  try
  {
    ...
    for (i=0; i<getNumPorts(); ++i){
      ...
      ports.at(i) = allocPort(tipo);
      if(!ports[i]->ler(myfile) || !validPort(i+1)) throw 5;
    ...
    for (i=0; i<getNumOutputs(); ++i)
    {
        ...
        myfile >> id>> c >> id_out.at(i);
        if(!myfile.good() || id != i+1 || c != ')' || !validIdOrig(id_out.at(i))) throw 7;
