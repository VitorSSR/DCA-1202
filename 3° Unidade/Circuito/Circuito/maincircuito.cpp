/*
ESTE PROGRAMA PERTENCE AOS ESTUDANTES:
-VALDEMAR GONCALVES PEREIRA NETO (20210036908)
-VITOR DOS SANTOS RODRIGUES (20210037020)
*/
...
void MainCircuito::slotNewCircuito(int NInputs, int NOutputs, int NPortas)
{
  ...
  if(NInputs < 0 || NOutputs <0|| NPortas < 0){
      QMessageBox::critical(this, tr("O Circuito não está completamente definido"),"Não pode ser salvo" );
  }
...
void MainCircuito::slotModificarSaida(int IdSaida, int IdOrigemSaida)
{
  ...
  if(!(C.validIdOrig(IdOrigemSaida) && C.validIdOutput(IdSaida))){
        QMessageBox::critical(this,tr("Saída de Circuito inválida"), "Não foi possivel muda a saída do circuito");
...
void MainCircuito::showOutput(unsigned i)
{
  ...
  // TEM QUE ALTERAR TODOS OS QUE ESTAO COM: PROVISORIAMENTE
