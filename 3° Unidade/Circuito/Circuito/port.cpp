...
// Se não for, retorna false; caso seja, retorna true.
...
void Port_NOT::digitar() {
    std::cout << "Digite o ID da entrada da porta NOT: ";
    std::cin >> id_in.at(0);

    while (id_in.at(0) == 0) {
        std::cerr << "ID invalido. Por favor, digite novamente: ";
        std::cin >> id_in.at(0);
    }
...
void Port_AND::simular(const std::vector<bool3S>& in_port)
{
    if (testValidSizeInputs(in_port)){
        ...
        for (unsigned int i = 1; i < getNumInputs(); ++i){
...
void Port_NAND::simular(const std::vector<bool3S>& in_port)
{
    if (testValidSizeInputs(in_port)) {
        ...
        out_port =~ out_port;  // Adicionando a negação para representar NAND
    } else {
        out_port = bool3S::UNDEF;
        return;
    }
...
void Port_NOR::simular(const std::vector<bool3S>& in_port){
    ...
    if (testValidSizeInputs(in_port)) {
        ...
        out_port =~ out_port;  // negação para representar NOR
